...
extends ItemSword {
    public ItemVoidSword(String unlocalizedName, Item.ToolMaterial material) {
        super(material);
        this.setTextureName("thetitans:void_sword");
    ...
    public void onPlayerStoppedUsing(ItemStack p_77615_1_, World p_77615_2_, EntityPlayer p_77615_3_, int p_77615_4_) {
        ...
        if (list1 != null && !list1.isEmpty()) {
            for (int i11 = 0; i11 < list1.size(); ++i11) {
                ...
                target.attackEntityFrom(DamageSourceExtra.causeAntiTitanDamage((Entity)p_77615_3_), 3000.0f * f);
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
        ...
        if (target != null) {
            ...
            if (target.height >= 6.0f || target instanceof EntityTitan || !target.onGround) {
                target.attackEntityFrom(DamageSourceExtra.causeAntiTitanDamage((Entity)attacker), 1500.0f);
                target.playSound("thetitans:titanpunch", 10.0f, 1.0f);
            }
        }
        return true;
