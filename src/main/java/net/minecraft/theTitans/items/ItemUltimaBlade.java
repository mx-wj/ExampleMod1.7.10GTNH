...
public class ItemUltimaBlade
extends ItemSword {
    public static Item.ToolMaterial enumToolMaterialUltima = EnumHelper.addToolMaterial((String)"Ultima", (int)Integer.MAX_VALUE, (int)2, (float)9999999.0f, (float)999996.0f, (int)0);
        ...
        if (list1 != null && !list1.isEmpty()) {
            for (int i1 = 0; i1 < list1.size(); ++i1) {
                ...
                if (player.canEntityBeSeen(entity)) {
                    player.addChatMessage((IChatComponent)new ChatComponentText("Ultima Blade: \u00a7l\u00a7oThe " + ((EntityLivingBase)entity).getCommandSenderName() + " is " + player.getDistanceToEntity(entity) + "\u00a7l\u00a7o blocks away from you."));
                    continue;
                }
                player.addChatMessage((IChatComponent)new ChatComponentText("Ultima Blade: \u00a7l\u00a7oAn unseen mob is located out of your \u00a7l\u00a7osight " + player.getDistanceToEntity(entity) + " blocks away from you."));
                player.addChatMessage((IChatComponent)new ChatComponentText("Ultima Blade: \u00a7l\u00a7oThe mob's health: " + ((EntityLivingBase)entity).getHealth() + "/" + ((EntityLivingBase)entity).getMaxHealth()));
                player.addChatMessage((IChatComponent)new ChatComponentText("Ultima Blade: \u00a7l\u00a7oThe mob's name: " + ((EntityLivingBase)entity).getCommandSenderName()));
        ...
        for (int l1 = -8; l1 <= 8; ++l1) {
            for (int i2 = -8; i2 <= 8; ++i2) {
                for (int j = -8; j <= 8; ++j) {
                    ...
                    player.addChatMessage((IChatComponent)new ChatComponentText("Ultima Blade: \u00a7l\u00a7oI sense a " + block.getLocalizedName() + "\u00a7l\u00a7o within atleast " + player.getDistance((double)j2, (double)k, (double)l) + " blocks of you."));
    ...
    public ItemStack onItemRightClick(ItemStack p_77659_1_, World p_77659_2_, EntityPlayer p_77659_3_) {
        ...
        if (p_77659_3_.inventory.hasItem(TitanItems.optimaAxe)) {
            ...
            if (list != null && !list.isEmpty()) {
                for (int i1 = 0; i1 < list.size(); ++i1) {
                    ...
                    ((EntityLivingBase)entity).addPotionEffect(new PotionEffect(ClientProxy.electricJudgment.id, Integer.MAX_VALUE, 19));
                    entity.addVelocity((double)(-MathHelper.sin((float)(p_77659_3_.rotationYaw * (float)Math.PI / 180.0f)) * 1.25f), 1.0, (double)(MathHelper.cos((float)(p_77659_3_.rotationYaw * (float)Math.PI / 180.0f)) * 1.25f));
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
        ...
        if (target != null) {
            ...
            if (target instanceof EntityTitan && ((EntityTitan)target).canBeHurtByPlayer()) {
                ...
                ((EntityTitan)target).setTitanHealth(((EntityTitan)target).getHealth() - 3000.0f - extradamage);
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (entityIn instanceof EntityPlayer && !worldIn.isRemote) {
            if (stack.getItemDamage() == 0 && entityIn.ticksExisted % 2 == 0) {
                ((EntityPlayer)entityIn).addChatMessage((IChatComponent)new ChatComponentText("Ultima Blade: You are unworthy of my power."));
                ((EntityPlayer)entityIn).inventory.consumeInventoryItem(TitanItems.ultimaBlade);
                ...
                if (((EntityPlayer)entityIn).inventory.hasItem(TitanItems.optimaAxe)) {
                    List list;
                    WorldServer worldserver = MinecraftServer.getServer().worldServers[0];
                    WorldInfo worldinfo = worldserver.getWorldInfo();
                    worldinfo.setRainTime(9999999);
        ...
        stack.setStackDisplayName("\u00a7lThe Ultima Blade");
        for (int i = 0; i < 3; ++i) {
            entityIn.worldObj.spawnParticle("fireworksSpark", ((EntityLivingBase)entityIn).posX + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * (double)((EntityLivingBase)entityIn).width, ((EntityLivingBase)entityIn).posY + ((EntityLivingBase)entityIn).getRNG().nextDouble() * (double)((EntityLivingBase)entityIn).height, ((EntityLivingBase)entityIn).posZ + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * (double)((EntityLivingBase)entityIn).width, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * 2.0, 1.0, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * 2.0);
            entityIn.worldObj.spawnParticle("explode", ((EntityLivingBase)entityIn).posX + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * (double)((EntityLivingBase)entityIn).width, ((EntityLivingBase)entityIn).posY + ((EntityLivingBase)entityIn).getRNG().nextDouble() * (double)((EntityLivingBase)entityIn).height, ((EntityLivingBase)entityIn).posZ + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * (double)((EntityLivingBase)entityIn).width, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * 2.0, 1.0, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5) * 2.0);
        }
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack) {
        ...
        if (!entityLiving.worldObj.isRemote && entityLiving instanceof EntityPlayer) {
            ...
            for (int i1 = 0; i1 < 24; ++i1) {
                ...
                dy = (double)entityLiving.getEyeHeight() + vec3.yCoord * (double)i1;
                dz = vec3.zCoord * (double)i1;
        ...
        for (int i1 = 0; i1 < 24; ++i1) {
            ...
            dy = (double)entityLiving.getEyeHeight() + vec3.yCoord * (double)i1;
            dz = vec3.zCoord * (double)i1;
            List list1 = entityLiving.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)entityLiving, entityLiving.boundingBox.expand(3.0, 3.0, 3.0).offset(dx, dy, dz));
            ...
            for (int i11 = 0; i11 < list1.size(); ++i11) {
                ...
                if (!((EntityTitan)entity1).canBeHurtByPlayer() && entityLiving instanceof EntityPlayer) {
                    ((EntityPlayer)entityLiving).addChatMessage((IChatComponent)new ChatComponentText(((EntityTitan)entity1).getCommandSenderName() + ": Quit flailing that giant sword at me."));
                }
                return true;
            }
        }
