...
extends ItemSword {
    public ItemGoodTurretGround(String unlocalizedName, Item.ToolMaterial material) {
        ...
        this.setTextureName("thetitans:machine_gun_good");
    }
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, int p_77648_4_, int p_77648_5_, int p_77648_6_, int p_77648_7_, float p_77648_8_, float p_77648_9_, float p_77648_10_) {
        ...
        double d0 = 1.0;
        int a = EnchantmentHelper.getEnchantmentLevel((int)TheTitans.turretEnchant1.effectId, (ItemStack)stack);
        int b = EnchantmentHelper.getEnchantmentLevel((int)TheTitans.turretEnchant2.effectId, (ItemStack)stack);
        int c = EnchantmentHelper.getEnchantmentLevel((int)TheTitans.turretEnchant3.effectId, (ItemStack)stack);
        int d = EnchantmentHelper.getEnchantmentLevel((int)TheTitans.turretEnchant4.effectId, (ItemStack)stack);
        int e = EnchantmentHelper.getEnchantmentLevel((int)TheTitans.turretEnchant5.effectId, (ItemStack)stack);
        int f = EnchantmentHelper.getEnchantmentLevel((int)Enchantment.unbreaking.effectId, (ItemStack)stack);
        ...
        for (int i = 0; i < 500; ++i) {
            entity.worldObj.spawnParticle("largesmoke", entity.posX + (rand.nextDouble() - 0.5) * ((double)entity.width * 3.0), entity.posY + rand.nextDouble() * ((double)entity.height + 1.0), entity.posZ + (rand.nextDouble() - 0.5) * ((double)entity.width * 3.0), 0.0, 0.0, 0.0);
        }
        if (!worldIn.isRemote) {
            worldIn.spawnEntityInWorld((Entity)entity);
        }
        if (entity != null && !playerIn.capabilities.isCreativeMode) {
            --stack.stackSize;
        }
        return true;
    }
